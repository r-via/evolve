...
files MUST NOT exceed 500 lines" cap.  Public symbols are still re-exported
from ``evolve.agent`` for backward compatibility with the existing test
suite (`patch("evolve.agent.run_spec_archival", ...)`).
"""
...
async def _run_spec_archival_claude_agent(
    ...
    # affect this callsite without import-time binding.
...
def run_spec_archival(
    ...
    # cycle (agent.py re-exports this module's public names).
