...
Extracted from ``evolve/reporting.py`` as part of the DDD restructuring
(SPEC.md § "Source code layout — DDD", migration step 13).
All callers continue to import via ``evolve.reporting`` (backward-compat shim)
or ``evolve.infrastructure.reporting`` (re-export ``__init__``).
"""
...
def _generate_evolution_report(
    ...
    _count_unchecked = _state_mod._count_unchecked
    _runs_base = _state_mod._runs_base
