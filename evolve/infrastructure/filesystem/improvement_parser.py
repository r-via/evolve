"""Improvement-parsing helpers — filesystem layer adapter.

Migrated from ``evolve/state_improvements.py`` as part of the DDD restructuring
(SPEC.md § "Source code layout — DDD", migration step 16).
All callers continue to import via ``evolve.state_improvements`` (backward-compat
shim) or ``evolve.infrastructure.filesystem`` (re-export ``__init__``).

This is a **leaf module** — it imports only stdlib (``pathlib`` / ``re``).
No imports from ``evolve.agent`` / ``evolve.orchestrator`` / ``evolve.cli`` /
``evolve.state``.
"""
