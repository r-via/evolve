"""State management — state.json, runs layout, convergence gates, backlog stats.

Migrated from ``evolve/state.py`` as part of the DDD restructuring
(SPEC.md § "Source code layout — DDD", migration step 14).
All callers continue to import via ``evolve.state`` (backward-compat shim)
or ``evolve.infrastructure.filesystem`` (re-export ``__init__``).
"""
...
def _compute_backlog_stats(
    ...
    if improvements_path.is_file():
        ...
        if rel_path:
            ...
            current_unchecked = _si._extract_unchecked_set(current_text)

            prev_text = _git_mod._git_show_at(project_dir, "HEAD", rel_path)
            if prev_text is not None:
                prev_unchecked = _si._extract_unchecked_set(prev_text)
                added_this_round = len(current_unchecked - prev_unchecked)

            five_back_text = _git_mod._git_show_at(project_dir, "HEAD~5", rel_path)
            if five_back_text is not None:
                five_back_pending = len(_si._extract_unchecked_set(five_back_text))
...
def _write_state_json(
    ...
    blocked_count = _si._count_blocked(improvements_path)
    ...
    backlog = _compute_backlog_stats(
        project_dir, improvements_path, remaining, done, blocked_count
    )
    ...
    state: dict = {
        ...
        "phase": phase,
        "status": status,
        "improvements": {"done": done, "remaining": remaining, "blocked": blocked_count},
        "backlog": backlog,
