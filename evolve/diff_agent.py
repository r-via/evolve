...
spec_archival split, US-032 (draft_review), US-033 (oneshot_agents),
and US-034 (sync_readme).

Public symbols (``build_diff_prompt``, ``_run_diff_claude_agent``,
``run_diff_agent``) are re-exported from ``evolve.oneshot_agents``
and ``evolve.agent`` for backward compatibility — existing tests
patch ``evolve.agent.X``, the orchestrator imports from
``evolve.agent``, and ``tests/test_oneshot_agents_module.py``
asserts each name is also bound under ``evolve.oneshot_agents``
(the re-export chain keeps both modules pointing at the SAME
object — ``is``-identical).
...
evolve/diff_agent.py`` returns zero matches.  Agent.py-resident
dependencies (``_load_project_context``, ``_run_agent_with_retries``)
and ``evolve.oneshot_agents``-resident dependencies
...
2. module-load order remains acyclic (memory.md round-7 entry:
   indented imports do NOT trip the leaf-invariant regex
   ``^from evolve\\.``);
3. the ``_run_readonly_claude_agent`` helper stays in
   ``evolve.oneshot_agents`` (shared by dry-run + validate + diff)
   and the lazy import preserves the test-patch surface.
