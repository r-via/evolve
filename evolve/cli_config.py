"""evolve CLI config-resolution helpers — extracted from ``evolve/cli.py``.

Hosts the four config-resolution symbols (``EFFORT_LEVELS``,
``_validate_effort``, ``_load_config``, ``_resolve_config``) per US-045
and SPEC.md § "Hard rule: source files MUST NOT exceed 500 lines".

``evolve/cli.py`` re-exports each symbol so existing imports
(``from evolve.cli import _resolve_config``, ``from evolve import
_resolve_config`` via the package shim, and ``patch("evolve.cli.
_resolve_config", ...)`` test targets) keep working unchanged — same
proven re-export pattern as US-039 (cli_utils), US-030 (agent_runtime),
US-031 (memory_curation), US-032 (draft_review), US-044
(state_improvements).
