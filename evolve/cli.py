"""evolve CLI — argument parsing, config resolution, and subcommand dispatch.

This module contains the ``main()`` entry point for the ``evolve`` command,
all argument parsing, config resolution helpers, and subcommand implementations
(init, start, status, history, clean, sync-readme, diff).

The ``pyproject.toml`` entry point is ``evolve.cli:main``.
"""
...
def _check_deps():
    ...
    """
    # __file__ is evolve/cli.py; project root is two levels up.
