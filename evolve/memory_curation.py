...
Extracted from ``evolve/agent.py`` to satisfy the SPEC § "Hard rule: source
files MUST NOT exceed 500 lines" cap (US-031, mirrors the US-027/US-030
extraction pattern).  Public symbols are still re-exported from
``evolve.agent`` for backward compatibility with the existing test suite
(``patch("evolve.agent.run_memory_curation", ...)``,
``patch("evolve.agent._run_agent_with_retries")`` from
``tests/test_memory_curation.py``, and the orchestrator's
``from evolve.agent import run_memory_curation`` callsite).
...
async def _run_memory_curation_claude_agent(
    ...
    """
    # Imported lazily so tests that patch ``evolve.agent.MODEL`` /
    # ``evolve.agent.EFFORT`` / ``evolve.agent.MAX_TURNS`` continue to
    # affect this callsite without import-time binding (mirrors
    # ``evolve.spec_archival._run_spec_archival_claude_agent``).
...
def run_memory_curation(
    ...
    # Lazy import of the agent-level retry helper to avoid an import-time
    # cycle (agent.py re-exports this module's public names).  Same pattern
    # as ``evolve.spec_archival.run_spec_archival``.
