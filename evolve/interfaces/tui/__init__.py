...
so that ``from evolve.interfaces.tui import …`` works as the canonical
import path for TUI code.
"""
...
def get_tui(
    *,
    ...
    """
    # _use_json may be set on the backward-compat shim module (evolve.tui)
    # by cli.py doing `import evolve.tui as m; m._use_json = True`.
    # Check both the canonical module and the shim.
    import sys
    json_flag = _use_json or getattr(
        sys.modules.get("evolve.tui"), "_use_json", False
    )
    if json_flag:
...
# Re-export classes so ``from evolve.interfaces.tui import RichTUI`` works.
