...
Extracted from ``evolve/agent.py`` per SPEC § "Hard rule: source files
MUST NOT exceed 500 lines" (agent.py was 601 lines; this extraction
brings it under cap).  Mirrors the leaf-module pattern established by
US-027 / US-030 / US-031 / US-032 / US-033 / US-034 / US-035.

**Leaf-module invariant.**  Top-level imports are limited to stdlib,
``evolve.agent_runtime``, ``evolve.state``, and ``evolve.tui``.  The
``EFFORT`` global is mutated at runtime by ``_resolve_config`` (SPEC §
"The --effort flag") and lives in ``evolve.agent`` — it MUST be
lazy-imported via ``from evolve.agent import EFFORT`` inside
``run_claude_agent``'s body so test-side ``agent.EFFORT = "max"``
mutations are honored AND the obvious circular import (agent.py
re-exports the symbols defined here at its module top) is avoided.
``claude_agent_sdk`` and ``evolve.costs`` are also lazy-imported at
function scope to keep the module loadable without the optional SDK.
"""
...
async def run_claude_agent(
    ...
    from evolve.agent import EFFORT
