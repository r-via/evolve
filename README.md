...
Party mode is blind to the TUI by default — its agents only see `README.md`,
...
- Recent captured TUI frames from `runs/<session>/frames/` (when
  `capture_frames = true` — see "Frame capture" above). The last 3-5 PNGs
  covering the final rounds + convergence are attached to each agent's
  prompt as image blocks, giving them visual context for the run.
