...
- [ ] [functional] In agent.py `run_claude_agent`, the conversation log file is opened without a context manager (`log = open(...)`) — if an exception occurs the file handle leaks. Refactor to use `with open(...) as log:` for proper resource cleanup
